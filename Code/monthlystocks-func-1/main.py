...
def monthly_stocks(request):
    secret_name = 'projects/alien-grove-405422/secrets/kaggleapi/versions/latest'
    ...
    return "Kaggle API authenticated, Monthly data is extracted, transformed and loaded into Postgres DB"
...
def connect_db_bulk(df,tbname):
    ...
    sql_query = "SELECT company, MAX(date) as date FROM public.monthly GROUP BY company ORDER BY company;"
...
def process_uploaded_files(bucket_name, storage_directory):
    ...
    for blob in monthly_blobs:
        ...
        if blob.name.endswith('.csv'):
            ...
            #Append the DataFrame to the 'monthly' list
