...
def daily_stocks(request):
    api = KaggleApi()
    api.authenticate()
    download_dataset(None)
    return "Kaggle API authenticated, Daily data is extracted, transformed and loaded into Postgres DB"
...
def connect_db_bulk(df,tbname):
    ...
    sql_query = "SELECT company, MAX(date) as date FROM public.daily GROUP BY company ORDER BY company;"
...
def process_uploaded_files(bucket_name, storage_directory):
    ...
    daily_blobs = [blob for blob in blobs if 'daily' in blob.name]
    print("Daily blobs: ", daily_blobs)
    for blob in daily_blobs:
        ...
        if blob.name.endswith('.csv'):
            ...
            data_dict["daily"].append(df.assign(company=company_updated))
    ...
    connect_db_bulk(df_daily_mapped,'daily')
    
    return len(df_daily_mapped)
